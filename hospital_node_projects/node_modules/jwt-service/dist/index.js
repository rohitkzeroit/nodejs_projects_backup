...
Finally, it deal with promises which are more convenient than the
 original API.
*/
export default service(initJWT, 'jwt', ['?JWT_SECRET_ENV_NAME', '?ENV', 'JWT', '?log', '?time']);
/**
 ...
 */
async function initJWT({ JWT_SECRET_ENV_NAME, ENV, JWT, time = Date.now.bind(Date), log = noop, }) {
    const JWT_DURATION = readMS(JWT.duration, 'E_BAD_JWT_DURATION');
    const JWT_TOLERANCE = readMS(JWT.tolerance, 'E_BAD_JWT_TOLERANCE', 0);
    const secretName = JWT_SECRET_ENV_NAME ||
        JWT.secretEnvName ||
        DEFAULT_JWT_SECRET_ENV_NAME;
