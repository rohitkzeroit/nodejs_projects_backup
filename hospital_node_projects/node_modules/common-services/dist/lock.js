...
*/
export default singleton(autoService(initLock));
/**
 ...
 * }
 */
async function initLock({ LOCKS_MAP = new Map(), LOCK_TIMEOUT = Infinity, delay, log = noop, }) {
    ...
    async function take(key, timeout = LOCK_TIMEOUT) {
        ...
        log('debug', `🔐 - Demanding the lock on "${key}" (queue length was ${locksLength})`);
        ...
        if (previousLock) {
            log('debug', `🔐 - Waiting the lock on "${key}" (queue length was ${locksLength})`);
            await previousLock.releasePromise;
        }
        log('debug', `🔐 - Obtaining the lock on "${key}" (queue length was ${locksLength})`);
    }
    /**
